...
void CuVectorBase<Real>::AddMatVec(const Real alpha,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    // transpose-ness.
    CU_SAFE_CALL(cublas_gemv(GetLocalCublasHandle(),
...
void CuVectorBase<Real>::AddSpVec(const Real alpha,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(cublas_spmv(GetLocalCublasHandle(), CUBLAS_FILL_MODE_UPPER, Dim(),
...
void CuVectorBase<Real>::CopyDiagFromMat(const CuMatrix<Real> &M) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    Timer tim;
    CU_SAFE_CALL(cublas_copy(GetLocalCublasHandle(), dim_, M.Data(), M.Stride() + 1,
			    data_, 1));
