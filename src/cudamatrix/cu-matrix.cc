...
template<typename Real>
void CuMatrixBase<Real>::PadFeatureMap(const CuMatrixBase<Real> &src, int num_input_fmaps, int fmap_x_len, int fmap_y_len,
		  int pad_x_len, int pad_y_len, int pad_wipe, int connect_fmap)
{
	int32 num_output_fmaps = num_input_fmaps;
    int32 max_col = std::max(src.NumCols(), NumCols());
    int32 min_col = std::min(src.NumCols(), NumCols());
	...
	KALDI_ASSERT(num_input_fmaps*fmap_x_len*fmap_y_len == min_col);
	KALDI_ASSERT(num_output_fmaps*(fmap_x_len+pad_x_len*2)*(fmap_y_len+pad_y_len*2) == max_col);
	...
	if (CuDevice::Instantiate().Enabled()) {
		...
		Timer tim;
		dim3 dimBlock(CU2DBLOCK, CU2DBLOCK);
		dim3 dimGrid(n_blocks(src.NumRows(), CU2DBLOCK), n_blocks(min_col, CU2DBLOCK));
		cuda_pad_feature_map(dimGrid, dimBlock, data_, src.data_, Dim(), src.Dim(),
				num_input_fmaps, fmap_x_len, fmap_y_len, pad_x_len, pad_y_len, pad_wipe, connect_fmap);
...
/////////////////////////////////////////////////////
template<typename Real>
void CuMatrixBase<Real>::CopyRowToVecId(CuArray<MatrixIndexT> &indexes)
{
		  	  	  ...
		  	  	  const CuArray<MatrixIndexT> &indices)
{
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    GetBlockSizesForSimpleMatrixOperation(A.NumRows(), A.NumCols(),
                                          &dimGrid, &dimBlock);
    cuda_add_mat_to_rows(dimGrid, dimBlock, alpha, data_, A.Data(), indices.Data(), Dim(), A.Dim());
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
#endif
  {
    Mat().AddMatToRows(alpha, A.Mat(), indices.Data());
  }
}
...
void ApplySoftMaxPerRowStreamed(std::vector<CuSubMatrix<Real>* > &des,
		const std::vector<CuSubMatrix<Real>* > &src, std::vector<CuSubVector<Real>* > *logsum)
{
	  KALDI_ASSERT(src.size() == des.size());
	  if ( logsum != NULL) KALDI_ASSERT(src.size() == logsum->size());
	  int32 size = src.size();
		...
		const CuArray<int32> &id_vec)
{
	  ...
	  if (CuDevice::Instantiate().Enabled()) {
	    ...
	    for (int32 i = 0; i < size; i++) {
	        dim3 dimBlock(CU2DBLOCK, CU2DBLOCK);
	        dim3 dimGrid(n_blocks(tgt[i]->NumCols(), CU2DBLOCK),
	                     n_blocks(tgt[i]->NumRows(), CU2DBLOCK));

	        cuda_gen_tgt(dimGrid, dimBlock, tgt[i]->Data(),
	                         id_vec.Data()+pos, tgt[i]->Dim(), tgt[i]->GetLocalCudaStream());
            pos += tgt[i]->NumRows();
	    }
	    CU_SAFE_CALL(cudaGetLastError());
	    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
...
#endif
	  {
		  for (int32 i = 0; i < size; i++) {
			  tgt[i]->Mat().SetZero();
			  for (int32 j = 0; j < tgt[i]->NumRows(); j++) {
				  tgt[i]->Mat()(j, id_vec.Data()[pos+j]) = 1.0;
			  }
              pos += tgt[i]->Mat().NumRows();
		  }
	  }
}

template
void GenTargetStreamed(std::vector<CuSubMatrix<float>* > &src,
	const CuArray<int32> &id_vec);

template
void GenTargetStreamed(std::vector<CuSubMatrix<double>* > &src,
	const CuArray<int32> &id_vec);
...
template<typename Real>
Real MatSumStreamed(const std::vector<CuSubMatrix<Real>* > &src) {
	  int32 size = src.size();
	  Real sum = 0.0;
	  ...
	  if (CuDevice::Instantiate().Enabled()) {
		...
		for (int32 i = 0; i < size; i++) {
			...
			cuda_row_sum_reduce(dimGrid, dimBlock, 1.0, value.Data()+pos, src[i]->Data(), src[i]->Dim(), 0.0, src[i]->GetLocalCudaStream());
			pos += src[i]->NumRows();
		}
		CU_SAFE_CALL(cudaGetLastError());
		sum = value.Sum();
...
template
float MatSumStreamed(const std::vector<CuSubMatrix<float>* > &src);
...
void ScaleStreamed(std::vector<CuSubMatrix<Real>* > &mat, std::vector<Real> &value)
{
	...
	{
		for (int32 i = 0; i < size; i++)
			mat[i]->Mat().Scale(value[i]);
	}
}

template
void ScaleStreamed(std::vector<CuSubMatrix<float>* > &mat, std::vector<float> &value);

template
void ScaleStreamed(std::vector<CuSubMatrix<double>* > &mat, std::vector<double> &value);
...
void AddStreamed(std::vector<CuSubMatrix<Real>* > &mat, std::vector<Real> &value)
{
	...
	{
		for (int32 i = 0; i < size; i++)
			mat[i]->Mat().Add(value[i]);
	}
}

template
void AddStreamed(std::vector<CuSubMatrix<float>* > &mat, std::vector<float> &value);
